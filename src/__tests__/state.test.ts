...
describe('loadState', () => {
  const statePath = '/root/.claude/delta-gate.state.local.worktree-specific.json';
  ...
  it('reads from the provided state file path', () => {
    mockReadFileSync.mockImplementation(() => { throw new Error('ENOENT'); });
    const customPath = '/my/project/.claude/delta-gate.state.local.worktree-specific.json';
